...
finding a handful of cybersecurity indicators (IPs, hashes, domains, emails, IDs) across many
Parquet log files. It builds a compact FST (finite state transducer) sidecar index next to
your Parquet — without modifying the files — that maps each value to the exact `(file,
row_group)` set containing it, and feeds that set into Apache DataFusion so a query reads only
the row groups that can match.
...
- **The index answer is authoritative on its own.** A single index lookup tells you an
  ...
  and that's something a probabilistic filter structurally cannot give you.
- **Flat query latency as the corpus grows.** PDQ touches only the matched file and, via
  ...
  file count; PDQ stays flat. On a 10/100/1000-file ladder its end-to-end latency moves ~5 → 7
  ms warm while DuckDB grows to ~200 ms — see [BENCHMARKS.md](BENCHMARKS.md).
...
- **Range and prefix, not just equality.** Because the FST is ordered, it serves prefix (subnet,
  path) and lexicographic range lookups — predicates a bloom filter can't express at all.
- **Non-invasive.** The index is a separate sidecar; your Parquet files are never rewritten.
...
- **Min/max statistics** only help when a column is sorted or clustered. On an unsorted
  ...
  one row group.
- **Bloom filters** fix equality on unsorted columns, but they're equality-only (no range or
...
- **Pruning decision (which row groups):** PDQ wins at every scale with **zero false positives**
  — an mmap'd FST traversal vs. opening and footer-parsing every file.
- **End-to-end, warm:** PDQ is flat (~5–7 ms across a 100× corpus growth) while the engines
  scale with file count — 28× faster than DuckDB at 1000 files. Tuning DuckDB's metadata cache
  moves it only −12%, so the gap is structural (PDQ is O(matches), engines are O(files)), not a
  footer-parsing artifact.
- **End-to-end, cold:** PDQ is flat (~15–17 ms) and wins at every scale. The headline is 16×
...
- **Cost:** the FST index is a separate sidecar (larger on disk than embedded blooms, ~6×) with
